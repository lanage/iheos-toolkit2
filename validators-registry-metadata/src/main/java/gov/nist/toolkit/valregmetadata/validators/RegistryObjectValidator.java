    ...
    public void validateExternalIdentifiersCodedCorrectly(ErrorRecorder er, ValidationContext vc, ClassAndIdDescription desc, String resource) {
        for (ExternalIdentifier ei : mo.getExternalIdentifiers()) {
            ...
            if (MetadataSupport.XDSDocumentEntry_uniqueid_uuid.equals(ei.getIdentificationScheme())) {
                ...
                // These comments are superseded by those below
                ...
                //System.out.println("\n\nmetadataValidationDocumentIDCodes: " + vc.metadataValidationDocumentIDCodes + "\n\n");

                // See https://profiles.ihe.net/ITI/TF/Volume3/ch-4.2.html#4.2.3.2.26
...
//                For documents using HL7v3 Instance Identifiers (e.g., CDAs) with an extension attribute, the uniqueId should be a serialization of the root and extension attributes in the form root^extension. The HL7v3 Instance Identifier URN encoding (using the namespace urn:hl7ii) should not be used.
                ...
                new OidFormat(er, mo.identifyingString() + ": " + ei.identifyingString(), externalIdentifierDescription(desc, ei.getIdentificationScheme()))
                        .validate(parts[0]);
                /*
                Superseded by 4.2.3.2.26 above
                ITI TF 3
